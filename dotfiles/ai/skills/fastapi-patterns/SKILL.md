...
description: Use when building or reviewing a FastAPI service - project structure, Pydantic v2 schemas, dependency injection, async handlers, authentication/authorization, transactional service layers, or httpx/pytest tests.
origin: ECC
---

# FastAPI Patterns

Production FastAPI dev. Covers project layout, Pydantic v2 schemas, dependency injection, async patterns, auth, transactional service methods, testing.
...
> **Note on Database Design:** App-level unique handling needs underlying unique DB index (e.g. `unique=True` on SQLAlchemy mapping attributes). No underlying constraints means app-layer error-catching cannot safely prevent concurrent race conditions.
...
- Always declare typed `response_model`. Prevents accidental PII/data leaks, outputs clean OpenAPI schemas.
- Consolidate middleware dependency injections via type-aliasing: `DbDep = Annotated[AsyncSession, Depends(get_db)]`.
- Wrap DB mutation boundaries in transactions inside service layer. Catch structural DB errors directly.
- Parse JWT params defensively. Expect string/integer cast mismatches from modern payload variations.
- Enforce deterministic sorting (e.g. `.order_by(Model.id)`) on all offset/limit paginated endpoints. Avoids data skips.
- Isolate authorization checks from core authentication dependencies. Gives precise REST status signals (`401` vs `403`).